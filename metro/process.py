...
for year in range(2008, 2020):
    ...
    # read raw data and rename columns to avoid problems with encoding
    ...
    # strip all unnecessary whitespace, commas, and columns
    for col in df.columns:
        df[col] = df[col].astype('str').str.replace(' ', '')
        df[col] = df[col].astype('str').str.replace(',', '')
    df = df.drop(columns=['날짜', '역명'], axis=1)

    # store the list of station numbers
    station_numbers = df['역번호'].unique().tolist()

    # separate the data into departure/arrival data
    # drop the column with boolean flag representing departure/arrival
    ...
    # separate the data for each station number
    # store the data into separate files
